...
def UVERTEX_BRELAZ(G,U,domains,k,coloration):
	...
	for node in U:
		dom_len = len(domains[node])
		domain_sizes[node] = dom_len

		if dom_len == min_len:
			# small_domains.append([node,0])
			small_domains[node] = 0
	...
	for sd,count in small_domains.items():
	...
	for sd,count in small_domains.items():
		...
		count_dict[count].append(sd)
		if count > max_count:
			max_count = count
	# try:

	return count_dict[max_count][random.randint(0,len(count_dict[max_count])-1)]		

	# except:
		# print("dom",dom_len)


	# sorted_sd = sorted(small_domains.items(), key=operator.itemgetter(1))
	# return sorted_sd[-1][0]	
...
def CVERTEX_BRELAZ(G,C,domains,n,k,coloration):
	...
	for sd,count in large_domains.items():
	...
	return count_dict[min_count][random.randint(0,len(count_dict[min_count])-1)]
		...
		if len(D) == 0:
			...
			for i in range(min(B,len(C))):
				...
				for neighbor in G.neighbors(c):
					...
					# if past_color not in domains[neighbor]:
				...
				# 	neighbor_colors.append(coloration[neighbor])
					...
					if past_color not in neighbor_colors2:	
						domains[neighbor].append(past_color)
...
# 	t0 = time.time()
# 	coloration = FCNS(G,1,k,verbose=False)
